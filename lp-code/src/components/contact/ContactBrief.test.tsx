...
function fill() {
  ...
  fireEvent.change(screen.getByLabelText("Seu e-mail para resposta"), {
    target: { value: "teste@example.com" },
  });
  fireEvent.change(screen.getByLabelText("Conte um pouco da sua ideia"), {
    target: { value: "Gostaria de construir um site." },
  });
...
describe("Contato CODE", () => {
  it("mostra canais provisórios sem simular envio quando falta a chave", () => {
    ...
    ).toHaveAttribute("href", "mailto:contato@juniorcode.com.br");
  });
  ...
  it.each(["rejected", "network"])(
    "preserva a mensagem quando há falha: %s",
    async (kind) => {
      vi.stubEnv("VITE_WEB3FORMS_ACCESS_KEY", "test-key");
      vi.stubGlobal(
        "fetch",
