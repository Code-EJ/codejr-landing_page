...
describe("ServiceCard", () => {
  it("exibe detalhes em diálogo nomeado e mantém o material compartilhado", () => {
    render(
      <ServiceCard
        title="Frontend"
        description="Interfaces acessíveis"
        deliverables={["APIs", "Testes"]}
        onClose={vi.fn()}
        className="custom-class"
      />,
    );
    expect(screen.getByRole("dialog", { name: "Frontend" })).toHaveClass(
      "liquid-glass",
      "custom-class",
    );
  ...
  it("fecha pelo botão e restaura o overflow ao desmontar", () => {
    ...
    const { unmount } = render(
      <ServiceCard title="Frontend" onClose={close} />,
    );
