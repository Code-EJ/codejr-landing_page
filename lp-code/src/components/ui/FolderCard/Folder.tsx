...
export function Folder({
  className,
  onClick,
  title = "Explorar serviço",
  description = "Conheça as possibilidades",
  index = 1,
}: Props) {
  ...
  return (
    <button
      type="button"
      className={cn(styles.folder, "group", className)}
      onClick={onClick}
      {...motion}
    ...
    >
      <span ref={art} className={styles.folderArt} aria-hidden="true">
        ...
        <span className={styles.paper} data-depth>
          <span />
          <span />
          <span />
        </span>
        <span className={`liquid-glass ${styles.front}`}>
          <span className={styles.folderCode}>
            {String(index).padStart(2, "0")} / CODE
          </span>
          <span className={styles.folderSymbol}>[ ]</span>
        </span>
      </span>
