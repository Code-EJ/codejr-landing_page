...
function Media({ item, reduced, onPlaying }: { item: CarouselItem; reduced: boolean; onPlaying: (playing: boolean) => void }) {
  ...
  if (failed) return (
    <div className={styles.fallback} role="status">
      ...
      <button type="button" className={styles.glassButton} onClick={() => setFailed(false)}>Tentar novamente</button>
    </div>
  );
    ...
    <section ref={rootRef} className={styles.carousel} style={{ maxWidth, height }} aria-label={label} aria-roledescription="carrossel"
      ...
      {!current ? <div className={styles.fallback} role="status">Nenhum conteúdo disponível.</div> : (
        <div ref={stageRef} className={styles.stage} data-single={total === 1 || undefined}
          ...
          {total > 1 && <button type="button" tabIndex={-1} className={styles.previewLeft} aria-label="Ver slide anterior" onClick={() => navigate(previous)}><div aria-hidden="true">{preview(validItems[(index - 1 + total) % total])}</div></button>}
          {total > 2 && <button type="button" tabIndex={-1} className={styles.previewRight} aria-label="Ver próximo slide" onClick={() => navigate(next)}><div aria-hidden="true">{preview(validItems[(index + 1) % total])}</div></button>}
      ...
      {total > 1 && (
        <div className={styles.controls}>
          <button type="button" className={styles.glassButton} aria-label="Item anterior" onClick={() => navigate(previous)}><span aria-hidden="true">←</span></button>
          ...
          <button type="button" className={styles.glassButton} aria-label="Próximo item" onClick={() => navigate(next)}><span aria-hidden="true">→</span></button>
          {!reduced && <button type="button" className={styles.playButton} aria-label={wantsPlay ? 'Pausar reprodução automática' : 'Iniciar reprodução automática'} onClick={() => { setPlayOverride(!wantsPlay); setFocusPaused(false); }}><span aria-hidden="true">{wantsPlay ? 'Ⅱ' : '▷'}</span></button>}
        </div>
      )}
